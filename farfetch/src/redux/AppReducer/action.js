...
const getSortData=(activeSort)=>(dispatch)=>{
   return axios(`http://localhost:8080/products?_sort=price&_order=${activeSort}`).then((r)=>{
    dispatch(getDataSuccess(r.data))
   })
}
const getmenData=()=>(dispatch)=>{
    return axios("http://localhost:8080/menData").then((r)=>{
        dispatch(getDataSuccess(r.data))
    })
}

const getwomenData=()=>(dispatch)=>{
    return axios("http://localhost:8080/womenData").then((r)=>{
        dispatch(getDataSuccess(r.data))
    })
}
